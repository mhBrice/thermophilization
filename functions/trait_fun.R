...
thermo_plot <- function(cti, reg, ylim = NULL, 
  ...
  if(lgd) legend("bottom", legend = c("Shift in warm species", "Shift in cold species"),
                 pch = 19, col = col[3:2], bty ="n")
...
plot_reg_cti <- function(delta_CTI, tbi, reg,
  ...
  for(i in 1:nreg) {
    ...
    if(i==1) mtext("∆CTI", 2, line = 1.8, cex = .8, xpd = NA, font = 2)
...
sp_contrib <- function(mat1, mat2, trait) {
